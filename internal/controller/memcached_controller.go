...
func (r *MemcachedReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil && errors.IsNotFound(err) {
		...
		log.Info("Creating a new Deployment", "Deployment.Namespace", sts.Namespace, "Deployment.Name", sts.Name)
	...
	size := memcached.Spec.MemcachedPoolsize
...
// statefulsetForMcrouter returns a memcached Deployment object
func (r *MemcachedReconciler) statefulsetForMcrouter(m *cachev1alpha1.Memcached) *appsv1.StatefulSet {
	ls := labelsForMemcached(m.Name)
	replicas := m.Spec.McrouterSize
